...
async def await_with_retry(
    ...
    log_prefix = log_prefix or f"{awaitable.__name__}: "
    errors = []
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if attempt + 1 < retries:
                ...
                if recover:
                    LOGGER.warning(f"{log_prefix}running recovery: {recover.__name__}")
                    try:
                        await recover()
                    except GiveUpError:
                        raise
                        ...
                        if not isinstance(recovery_error, catch_exceptions):
                            raise
                        LOGGER.warning(
                            f"{log_prefix}recovery failed with {recovery_error!r}, "
                            "will retry main operation"
                        )

    # All attempts exhausted
    LOGGER.warning(f"{log_prefix}giving up after {retries} attempts")
